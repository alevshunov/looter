    ...
    getNextShop(): Promise<Shop> {
        return new Promise(async (resolve, reject) => {
            ...
            const result = await conn.query(
                "select * from shops where active and (last_fetch < date_add(now(), interval -6 hour) or last_fetch is null) and date < date_add(now(), interval -1 minute) order by last_fetch asc, id desc limit 1",
                );
